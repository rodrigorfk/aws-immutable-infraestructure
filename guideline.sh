...
# setup básico da conta AWS
aws configure --profile my-aws-admin-user
export AWS_PROFILE=my-aws-admin-user
export AWS_DEFAULT_REGION=us-west-2

# cria usuário no IAM da AWS com permissões necessárias para executar o packer build e posteriormente criação de cluster do Kubernetes
...
# faz o build da primeira AMI - uma imagem básica do centos
cd packer/centos-base
packer validate packer-template.json
packer build packer-template.json 

# faz o build da segunda AMI a partir da primeira
